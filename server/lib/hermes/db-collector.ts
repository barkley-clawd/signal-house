 ...
 * `normalizeModelName` reads. The hermes state.db stores `providerID`
 * as a sibling of `id`/`modelID`/etc. inside `sessions.model`, but the
 * two-step `normalizeModelName → sharedNormalizeModelName` pipeline
 * only derives provider from a `provider/model` slash prefix on the
 * string — it never reads the JSON `providerID` field. Returning it
 * here lets `queryModelBreakdown` surface the real provider for every
